...
/** Approximate next run from calendar + startTime (local browser TZ; good enough for display) */
function computeNextRun(job) {
  ...
  for (let i = 0; i < 400; i++) {
    if (candidate > now && matches(candidate)) {
      return formatNextRun(candidate);
    }
...
function normalizeScheduledJob(j) {
  ...
  return {
    ...
    type,
    enabled,
    nextRunDisplay: computeNextRun({ ...j, calendar, startTime: startTime === '—' ? '' : startTime })
  };
}
...
/* ========== SCHEDULED JOBS (Automic-style table) ========== */
...
function renderScheduledTable() {
  ...
  if (!page.length) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:40px;color:#94a3b8">No scheduled jobs found</td></tr>`;
  } else {
    page.forEach(j => {
      const tr = document.createElement('tr');
